...
def load_data_train_model(run, data, CONV_INPUT_LENGTH, write_preds=False):
    ...
    base_model = sf.GlucoseModel(CONV_INPUT_LENGTH, True, run)
    ...
    for i in patients_to_include:
        ...
        if write_preds:
            ...
            train_preds.to_csv(
                os.path.join(
                    "preds", f"base_9_train_M{run.params.missingness_modulo}_D{i}.csv"
                )
            )
            test_preds.to_csv(
                os.path.join(
                    "preds", f"base_9_test_M{run.params.missingness_modulo}_D{i}.csv"
                )
            )
...
if __name__ == "__main__":
    ...
    if args.experiment:
        ...
        experiment = sigopt.create_experiment(
            ...
            type="grid",
            parameters=[dict(name="missingness_modulo", type="int", grid=[800, 1000])],
        ...
        experiment = sigopt.create_experiment(
            ...
            parameters=[
                ...
                dict(
                    ...
                    type="double",
                    bounds=dict(min=0.0001, max=0.002),
                ),
                dict(
