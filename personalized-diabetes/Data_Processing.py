...
def preprocess_data(test: bool = False):
    ...
    df_MonitorCorrectionBolus = load_table(filename='MonitorCorrectionBolus',
                                           date_cols=tables['MonitorCorrectionBolus'])
    ...
    df_MonitorSystem = df_MonitorSystem[
        ['DeidentID', 'LocalDtTm', 'DiAsState', 'IOBValue', 'Hypolight', 'Hyperlight', 'Exercising']]
    ...
    for table in investigation_tables.keys():
        df = get_hourly_coverage(investigation_tables[table][0], 'LocalDtTm', 'DeidentID',
    ...
    for table in list(result_tables.keys())[1:]:
        res_df = res_df.merge(result_tables[table], on=['DeidentID', 'LocalDtTm'], how='outer',
                              suffixes=('', f'_{table}'))
    ...
    for i in range(1, 31):
        ...
        print('Starting insulin, mealsize, carbs and exercise calculations for patient: ', i, '...')
        ...
        print('Finished insulin, mealsize, carbs and exercise calculations for patient: ', i,
        ...
        print(f'Self-supervised and basic dfs successfully constructed for patient {DeidentId}')
